...
class FlexField extends DefaultField{
    ...
    public function render(){
        ...
        $class = 'flex-field repeater-field';
    ...
    public function build(){
        ...
        if( !empty( $values ) ){
            ...
            foreach( $values as $id => $value ){
                ...
                $this->makeLayout( $value, $id, false );
                $i++;
            }
     ...
     * @return String
     */
    public function makeLayout( $value, $id, $doingAjax = true ){

        $prefix = $this->name."[{$id}]";
        $layout = $this->layouts[ $value['layout'] ];

        echo '<div class="repeatable flex-layout">';

            echo '<h3>'.$layout['label'].'</h3>';
     ...
     * 
     */
    public function addLayoutButton()
    {
        echo '<div class="add-layout-button-wrapper">';
            
            echo '<button id="add-layout" class="button button-primary button-large">';
                echo '<span class="dashicons dashicons-plus-alt"></span>';
                echo $this->getProperty( 'button_text', __( 'Add a block', 'cuisine') );
            echo '</button>';

            echo '<ul class="layout-selector chosen">';
            foreach( $this->layouts as $value ){
                echo '<li class="add-layout" data-layout="'.$value['layout'].'">';
                    echo $value['label'];
                echo '</li>';
            }
            echo '</ul>';
    ...
    public function renderPrivateTemplate(){
        ...
        foreach( $this->layouts as $layout ){
            ...
            $layoutSlug = $layout['layout'];

            //make a clonable item, for javascript:
            echo '<script type="text/template" id="'.$this->getTemplateName().'-'.$layoutSlug.'">';

                $this->makeLayout( $layout, '<%= highest_id %>' );
    ...
    private function buildDatas(){
        ...
        $datas .= 'data-max-repeats="'.$this->getProperty( 'maxItems', -1 ).'" ';
        $datas .= 'data-min-repeats="'.$this->getProperty( 'minItems', -1 ).'" '; 
    ...
    private function buildControls(){
        ...
        echo '<div class="repeat-controls layout-controls">';

            echo '<div class="min btn"><span class="dashicons dashicons-minus"></span></div>';
            echo '<div class="sort-pin btn"><span class="dashicons dashicons-sort"></span></div>';
